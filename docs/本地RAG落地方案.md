...
- `AI_Agent.spec` —— datas 纳入模型文件 + tokenizers hook
...
- 模型 ~30MB **直接纳入** PyInstaller(`AI_Agent.spec` 的 datas)，**不按需下载**。
...
- 实现并扩展 `docs/整体计划.md` 待办 #2「笔记问答(本地 RAG)」→ 升级为本地知识库。
- 与 `docs/功能深化与扩展建议.md` 的 #1(检索升级)、#2(NoteTool 补全)合并推进。
